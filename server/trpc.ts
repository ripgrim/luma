...
import { type NextApiRequest, type NextApiResponse } from 'next'; // For type checking
import { type NextRequest } from 'next/server'; // For App Router, Request is also a global type
...
const isAuthenticatedMiddleware = t.middleware(({ ctx, next }) => {
  ...
  if (!ctx.user || !ctx.user.id) { 
    throw new TRPCError({ code: 'UNAUTHORIZED', message: 'User not authenticated. Ensure session retrieval is correctly implemented in tRPC context.' });
  }
  return next({
    ctx: {
      ...ctx,

        ...
        .query(async ({ input }) => {
            try {
                ...
                if (idsToFetch.length > 0) {
                    console.log(
                        `[robloxItems.getItemThumbnails] Fetching thumbnails for ${idsToFetch.length} items`
                    )
                    ...
                    if (!response.ok) {
                        console.error(
                            `[robloxItems.getItemThumbnails] Error fetching item thumbnails: ${response.status} ${response.statusText}`
                        )

        ...
        .query(async ({ input }) => {
            try {
                ...
                if (idsToFetch.length > 0) {
                    ...
                    console.log(
                        `[robloxAvatars.getUserAvatars] Fetching avatars for ${idsToFetch.length} users`
                    )
                    ...
                    if (!response.ok) {
                        console.error(
                            `[robloxAvatars.getUserAvatars] Error fetching Roblox avatars: ${response.status} ${response.statusText}`
                        )

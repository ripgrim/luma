    ...
    .mutation(async ({ input }) => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Roblox API Error: ${errorData?.errors?.[0]?.message || errorData?.message || response.statusText}`);
        }
        ...
        if (!code || !privateKey || !status || !expirationTime) {
          ...
          throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Incomplete data from Roblox API.'});
        }
        ...
        let message = error.message || 'Failed to create session';
  ...
  checkQuickLoginStatus: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!activeSessions[sessionId]) {
        ...
        throw new TRPCError({ code: 'NOT_FOUND', message: 'Invalid or expired session ID.' });
      }
      ...
      if (session.status === 'Validated' || session.status === 'Cancelled' || session.status === 'Expired') {
      ...
      try {
        ...
        try {
          ...
          console.error('Failed to get CSRF token, but continuing with null token:', csrfError);
        ...
        try {
          ...
          console.error('Error processing response data:', processingError, 'Data:', responseData);
          return {
            success: false,
            status: session.status,
  ...
  completeQuickLoginAndStoreCookie: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        if (existingUser && existingUser.length > 0 && existingUser[0].roblosecurity_cookie) {
          console.log('User already has Roblosecurity cookie, might be a duplicate request');
          return {
            success: true,
      ...
      if (!activeSessions[sessionId]) {
        ...
        try {
          ...
          if (existingUser && existingUser.length > 0 && existingUser[0].roblosecurity_cookie) {
            return {
              success: true,
              message: 'Already authenticated with Roblox (session expired but cookie exists)',
              cookieStored: true,
            };
          }
        } catch (dbError) {
          console.error('Error checking for existing cookie after session not found:', dbError);
        }
      ...
      if (session.userId && session.userId !== userId) {
        console.warn(`Session ${sessionId} was initiated for user ${session.userId} but completion attempted by ${userId}`);
      ...
      try {
        ...
        if (cookies) {
          ...
          if (cookieMatch && cookieMatch[1]) {
            ...
            console.log('Found ROBLOSECURITY cookie, length:', roblosecurityCookieValue.length);
          } else {
        ...
        if (!roblosecurityCookieValue) {
          console.error("No .ROBLOSECURITY cookie found in response from Roblox /v2/login");
          ...
          try {
            ...
            if (responseBody.includes('already authenticated') || 
  ...
  cancelQuickLoginSession: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const response = await fetch('https://apis.roblox.com/auth-token-service/v1/login/cancel', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-csrf-token': csrfToken
  ...
  logoutRobloxUserSession: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!dbUser || dbUser.length === 0 || !dbUser[0].roblosecurity_cookie) {
        ...
        return { success: false, message: 'No Roblosecurity cookie found for this user or user not found.' };
      }
      ...
      try {
        ...
        if (response.status === 403) {
          console.error('Forbidden response from Roblox logout. This is common and we can still clear the local cookie.');
...
setInterval(() => {
  ...
  for (const [sessionId, session] of Object.entries(activeSessions)) {
    try {
      ...
      console.error(`Error parsing expirationTime for session ${sessionId}, removing from active sessions. Error: ${e}`);
      delete activeSessions[sessionId];
    }
  }

...
import type { db } from "@/database/db" // Assuming db is accessible here for the helper
...
export async function getRoblosecurityTokenForUser(
    userId: string,
    ...
    if (!userRecord || userRecord.length === 0 || !userRecord[0]?.roblosecurity_cookie) {
        console.error(
            "[getRoblosecurityTokenForUser] Roblosecurity cookie not found for user:",
            userId
        )
        throw new TRPCError({
            code: "UNAUTHORIZED",
            message:
                "Roblosecurity cookie not found for the user. Please ensure it is set in your profile."
        })
    }

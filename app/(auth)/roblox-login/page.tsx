    ...
    return (
        <div className="mt-3 mb-0 w-full">
            <div className="mb-1.5 flex justify-between">
                ...
                <span
                    className={`font-bold font-mono text-sm ${isExpired ? "text-destructive" : "text-primary"}`}
                >
                    {timeLeft}
                </span>
            </div>
    ...
    if (!session) {
        return (
            <div className="flex min-h-[70vh] flex-col items-center justify-center p-4">
                <Card className="roblox-auth-card w-full max-w-md">
                    <CardHeader>
                        <div className="flex flex-col items-center gap-2 p-4">
                            <RobloxIcon className="h-20 w-20" />
                            <CardTitle className="text-xl">Roblox Login</CardTitle>
                        </div>
        ...
        <div className="flex min-h-[70vh] flex-col items-center justify-center p-4">
            <Card className="roblox-auth-card w-full max-w-md">
                <CardHeader>
                    ...
                    {statusMessage && (
                        <CardDescription
                            className={`text-center ${loginStatus === "error" ? "text-destructive" : ""}`}
                        >
                            {statusMessage}
                        </CardDescription>
                    )}
                ...
                <CardContent>
                    {loginStatus === "waiting" && (
                        <div className="flex flex-col items-center py-8">
                            ...
                            <div className="h-8 w-8 animate-spin rounded-full border-4 border-t-primary border-r-transparent border-b-transparent border-l-transparent" />
                        </div>
                    )}
                    ...
                    {loginStatus === "pending" && qrCode && (
                        <div className="flex flex-col items-center">
                            <p className="mb-4 text-center text-muted-foreground">
                                Scan this QR code with the Luma companion app
                            </p>
                            <div className="mb-4 rounded-lg border border-border bg-white p-4">
                                <Image
                                    src={qrCode}
                                    alt="Roblox login QR code"
                                    width={200}
                                    height={200}
                                />
                            </div>

                            <div className="mb-4 w-full rounded-lg bg-muted p-3 text-center">
                                <p className="mb-1 text-muted-foreground text-sm">
                                    Or enter this code in the Luma companion app:
                                </p>
                                <p className="font-bold font-mono text-lg tracking-wider">
                                    {qrCode.includes("code=")
                                        ? new URL(qrCode).searchParams.get("code")
                                        : "Loading..."}
                                </p>
                            </div>
                    ...
                    {loginStatus === "validated" && (
                        <div className="flex w-full flex-col items-center py-8">
                            <p className="mb-4 w-full break-words px-2 text-center text-muted-foreground">
                                Finishing up...
                            </p>
                            <div className="h-8 w-8 animate-spin rounded-full border-4 border-t-primary border-r-transparent border-b-transparent border-l-transparent" />
                        </div>

...
export function RobloxItemsProvider({ children }: { children: ReactNode }) {
  ...
  const fetchThumbnails = useCallback(async (assetIds: string[]) => {
    if (!assetIds.length) return

    try {
      console.log(`[RobloxItemsProvider] Fetching thumbnails for ${assetIds.length} items`)
  ...
  const getItemThumbnailUrl = useCallback((assetId: string): string | null => {
  ...
  const preloadItemThumbnails = useCallback((assetIds: string[]) => {
    if (!assetIds.length) return
  ...
  return (
    <RobloxItemsContext.Provider value={contextValue}>
      {children}
    </RobloxItemsContext.Provider>
  )
}

...
import { ChevronDown, X as LucideX, Scale, TrendingUp } from "lucide-react"
...
interface SideData {
    ...
    total: number
    user?: {
        // Optional user receiving the items
        name: string
...
function ItemSlot({ item }: { item: ItemData }) {
    return (
        <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-lg bg-zinc-800">
            ...
            />
            <div className="absolute right-0 bottom-0 left-0 bg-black/70 px-1.5 py-0.5">
                <p className="truncate text-center font-medium text-[10px] text-white">
                    {item.name}
                </p>
            </div>
        </div>
...
export default function TradeComposer({ className }: { className?: string }) {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (
                sendOptionsDropdownRef.current &&
                !sendOptionsDropdownRef.current.contains(event.target as Node)
            ) {
    ...
    const displayedKnownContacts = knownContactsData.filter(
        (kc) => !selectedRecipients.find((sr) => sr.name === kc.name)
    )
    ...
    return (
        ...
        >
            <div className="relative flex items-center justify-center">
                {showSummary && (
                    <div className="absolute top-[160px] left-0 z-20 flex-col items-start justify-start gap-3 overflow-hidden rounded-xl border border-[#8B5CF6] bg-[#2A1D48] p-4 shadow-lg">
                ...
                <div className="relative z-10 mx-auto w-full flex-col overflow-hidden rounded-xl bg-[#1A1A1A] shadow-lg">
                    <div className="flex h-auto min-h-[40px] flex-wrap items-center gap-2 border-zinc-800/50 border-b px-4 py-1.5">
                        <div className="shrink-0 text-sm text-zinc-500">Send trade to:</div>
                        <div className="flex min-w-[200px] flex-1 flex-wrap items-center gap-2">
                            {selectedRecipients.map((recipient) => (
                                ...
                                >
                                    ...
                                    <button
                                        onClick={() => handleRemoveRecipient(recipient.name)}
                            ...
                            <div className="relative flex-grow">
                                <input
                                    ...
                                    className="w-full bg-transparent py-1 text-sm text-white placeholder-zinc-500 focus:outline-none"
                                />
                                {showRecipientDropdown &&
                                    (displayedKnownContacts.length > 0 ||
                                        ...
                                        <div
                                            ...
                                            className="absolute top-full left-0 z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-md border border-zinc-700 bg-zinc-800 shadow-lg"
                                        >
                                            {displayedKnownContacts.length > 0 && (
                                                <div>
                                                    <div className="px-3 py-2 font-semibold text-xs text-zinc-400">
                                                        Known Contacts
                                                    </div>
                                            ...
                                            {filteredRecipients.length > 0 && (
                                                <div>
                                                    {displayedKnownContacts.length > 0 && (
                                                        <div className="mx-3 my-1 h-px bg-zinc-700" />
                                                    )}
                                                    <div className="px-3 py-2 font-semibold text-xs text-zinc-400">
                                                        Results
                                                    </div>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="relative" ref={sendOptionsDropdownRef}>
                                <div className="flex items-center justify-start rounded-md bg-white text-black">
                                    ...
                                    <button
                                        onClick={() =>
                                            setShowSendOptionsDropdown(!showSendOptionsDropdown)
                                        }

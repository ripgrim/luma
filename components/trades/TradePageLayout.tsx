...
import { TradePageSkeleton } from "@/components/skeletons/trade"
import { TradeStatusType } from '@/server/types/robloxTrades'
...
function TradePageContent({ initialTradeType }: { initialTradeType: TradePageLayoutProps["tradeType"] }) {
  ...
  const handleTradeTypeChange = useCallback((newType: string) => {
  ...
  const { isLoading: isInitialLoading, data: initialTradesData } = trpc.robloxTrades.getStoredTrades.useQuery(
    { 
      tradeType,
      limit: PAGE_SIZE,
    },
  ...
  useEffect(() => {
    if (initialTradesData?.trades && initialTradesData.trades.length > 0) {
      ...
      setHasMore(initialTradesData.nextCursor !== null || initialTradesData.trades.length >= PAGE_SIZE);
  ...
  }, [nextCursor, hasMore, isLoadingMore, tradeType, utils.robloxTrades.getStoredTrades, queueAvatarLoading]);
  
  // Sort trades by date (newest first)
  const sortedTrades = [...trades].sort((a, b) => new Date(b.created).getTime() - new Date(a.created).getTime())
  ...
  const handleTradeClick = (trade: Trade) => {
    ...
    if (trade.originalId) {
      ...
      if (tradeItems[trade.id] && tradeItems[trade.id].length > 0) {
        console.log(`Already have ${tradeItems[trade.id].length} items for trade ${trade.id}, but refreshing data.`);
      }
      
      // Fetch detailed trade data to get userAssets
      utils.robloxTrades.getTradeDetails.fetch({ 
        ...
        if (result.success && result.trade) {
          ...
          if (result.trade.offers && Array.isArray(result.trade.offers)) {
            result.trade.offers.forEach((offer, index) => {
              console.log(`Offer ${index} ${index === 0 ? '(Your offer)' : '(Partner offer)'}`);
              ...
              if (offer.userAssets && Array.isArray(offer.userAssets)) {
                ...
                console.table(offer.userAssets.map(asset => ({
                  id: asset.id,
                ...
                offer.userAssets.forEach(asset => {
                  items.push({
                    id: asset.id?.toString() || `asset-${trade.originalId}-${asset.assetId}-${Math.random().toString(36).substring(2, 9)}`,
            ...
            console.log(`Items by offer type:`, {
              ...
              partner_offer: items.filter(item => item.offerType === 'partner_offer')
            });
        ...
        if (result.trade.offers && Array.isArray(result.trade.offers)) {
          result.trade.offers.forEach((offer, index) => {
            ...
            if (offer.userAssets && Array.isArray(offer.userAssets)) {
              offer.userAssets.forEach(asset => {
                items.push({
                  id: asset.id?.toString() || `asset-${selectedTrade.originalId}-${asset.assetId}-${Math.random().toString(36).substring(2, 9)}`,
                  assetId: asset.assetId?.toString(),
                  assetName: asset.name,
                  serialNumber: asset.serialNumber,
                  recentAveragePrice: asset.recentAveragePrice,
          ...
          if (items.length > 0) {
            console.log(`[Auto-fetch] Got ${items.length} items for trade ${tradeKey}`);
  ...
  const LoadMoreButton = () => (
    <div className="py-4 flex justify-center">
      {hasMore ? (
        ...
        <p className="text-xs text-muted-foreground">End of trades</p>
      )}
    </div>
  );
    ...
    return (
      <Drawer.Root open={isDrawerOpen} onOpenChange={setIsDrawerOpen}>
        ...
        <Drawer.Portal>
          ...
          <Drawer.Content className="bg-background flex flex-col rounded-t-[10px] h-[85%] mt-10 fixed bottom-0 left-0 right-0 z-50 focus:outline-none shadow-lg">
  ...
  if (selectedTrade) {
    console.log(`Rendering TradeDisplay for trade ${selectedTrade.id} with ${tradeItems[selectedTrade.id]?.length || 0} items`);
  }

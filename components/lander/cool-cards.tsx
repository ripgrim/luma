...
import { Clock, Rocket, Code, Terminal, Network, Globe, Settings, Tag, Workflow, Database, Globe2, ArrowRight, Layers, Server } from "lucide-react";
import clsx from "clsx";
import Image from "next/image";

const WT = ({ children }: { children: React.ReactNode }) => <span className="text-white/90">{children}</span>;
				...
				<div className="mt-auto">
					...
					<div className="px-8 pb-8 relative z-10">
						<div className="flex items-center justify-end text-white opacity-0 group-hover:opacity-40 transition-opacity">
							<ArrowRight className="text-xl -translate-x-1 group-hover:translate-x-0 transition-all" />
						</div>
					</div>
				</div>
			</div>

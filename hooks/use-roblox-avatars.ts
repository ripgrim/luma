...
export function useRobloxAvatars() {
  ...
  const queueAvatarLoading = useCallback((userIds: string[], isPriority = false) => {
    ...
    setIsLoading(true);
  }, [avatarMap]); // Only depend on avatarMap to check what's already loaded
  ...
  const processPendingAvatars = useCallback(() => {
    if (pendingAvatarsRef.current.size > 0 && !isLoading) {
      ...
      const idsToLoad = pendingIds.filter(id => 
        !avatarMap[id] && !globalAvatarCache[id] && !processedRef.current.has(id)
      );

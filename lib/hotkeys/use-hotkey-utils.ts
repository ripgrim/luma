...
export function useShortcuts(
    ...
    const effectiveReactHotkeyOptions: Options = {
        ...
        ...(hookOptions
            ? {
                  enableOnFormTags:
                      hookOptions.enableOnFormTags === undefined
                          ? defaultHotkeyOptions.enableOnFormTags
                          : hookOptions.enableOnFormTags,
                  preventDefault:
                      hookOptions.preventDefault === undefined
                          ? defaultHotkeyOptions.preventDefault
                          : hookOptions.preventDefault,
    ...
    if (hookOptions) {
        for (const key in effectiveReactHotkeyOptions) {
            if (
                effectiveReactHotkeyOptions[key as keyof Options] === undefined &&
                defaultHotkeyOptions[key as keyof Options] === undefined
            ) {
    ...
    shortcuts.forEach((shortcut) => {
        ...
        if (handler) {
            ...
            if (process.env.NODE_ENV === "development") {
                console.warn(
                    `[useShortcuts] No handler defined for shortcut: "${shortcut.id}" in scope: "${effectiveReactHotkeyOptions?.scopes || "unknown"}"`
                )
            }
        }
    })

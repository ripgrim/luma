...
export function GlobalHotkeys() {
  ...
  try {
    ...
    console.log(`[GlobalHotkeys] Initializing for scope: global. Sidebar context found. Sidebar open: ${sidebarControls.open}`);
  } catch (error) {
    console.log('[GlobalHotkeys] Initializing for scope: global. Sidebar context NOT found. Sidebar hotkeys will log warnings if used.');
  }
  ...
  const handlers = {
    search: (event: KeyboardEvent) => {
      ...
      if (searchInput) {
        ...
        console.warn('[GlobalHotkeys] Search Hotkey: No common search input found on the page.');
      }
    },
    ...
    collapseSidebar: (event: KeyboardEvent) => {
      if (sidebarControls && sidebarControls.toggleSidebar) {
        console.log(`[GlobalHotkeys] collapseSidebar handler triggered. Current state: ${sidebarControls.open ? 'open' : 'collapsed'}. Toggling.`);
        sidebarControls.toggleSidebar();
      } else {
        console.warn('[GlobalHotkeys] collapseSidebar hotkey triggered, but toggleSidebar function is not available. Is SidebarProvider an ancestor and context found?');
      }
    },
  };
